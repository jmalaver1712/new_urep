...
<head>
  ...
  $noticia_evento = $get_xml->reg_detail_xml("name" ,$_GET['id'] ,$_GET['name'].".xml", 1);
...
<div class="breadcrumb"><a href="<?= URL_MASTER ?>/pages/"> Inicio </a> > Publicaciones > <a href="<?= URL_MASTER ?>/pages/<?= $_GET['name'] ?>/" ><?= $_GET['name'] ?></a> > <a href="<?= URL_MASTER ?>/pages/publicaciones/<?= $_GET['name'] ?>/<?= $noticia_evento[0]->tipo ?>" ><?= $noticia_evento[0]->tipo ?></a> > <?= $noticia_evento[0]->titular ?></div>
  <!-- CONTENIDO -->
  <div class="container">
    <center>
      <h3 style="text-transform: uppercase;"><?= $noticia_evento[0]->titular ?></h3>
    </center>
